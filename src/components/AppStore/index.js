...
class AppStore extends Component {
  state = {searchInput: '', activeTableId: tabsList[0].tabId}

  setActiveTable = tabId => {
    this.setState({activeTableId: tabId})
  }

  onChangingSerchInput = event => {
    this.setState({searchInput: event.target.value})
  }

  getActiveTabsApp = searchedApps => {
    const {activeTableId} = this.state
    const filteredApp = searchedApps.filter(
      eachserchedapp => eachserchedapp.category === activeTableId,
    )
  ...
  render() {
    ...
    const searchResult = this.getSerchResult()
    const filteredApp = this.getActiveTabsApp(searchResult)
        ...
        <div className="app-store">
          ...
          <div className="serch-input-container">
            <input
              ...
              value={searchInput}
              onChange={this.onChangingSerchInput}
            />
            <img
              src={SEARCH_ICON_URL}
              alt="search icon"
          ...
          <ul className="tabs-list">
            {tabsList.map(eachtab => (
              <TabItem
                ...
                isActive={eachtab.tabId === activeTableId}
              />
            ))}
          </ul>
